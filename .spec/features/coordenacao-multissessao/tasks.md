...
## T-039 - Alvo ja consumido por um verbo anterior nao identifica o proximo kill [concluida]
- Refs: AC-028
- Arquivos: src/ccoord/classify.py, tests/test_classify.py
- Origem: 9a auditoria adversarial (21/09), achado ALTA. A janela de `_alvo_a_esquerda_do_verbo` atravessa `;`/`&&`/`||` de proposito -- e o que faz o idioma do CIM (`Get-CimInstance ...; $p | ForEach-Object { ... }`) ser identificado. Com mais de um verbo, o segundo herdava o alvo JA atribuido ao primeiro, voltava nao-None e escapava do sentinela: o comando inteiro saia `allow` pelo alvo ja aprovado. Medido ponta a ponta com `classify()` + `decide()`, e reproduzido tambem contra o `src` instalado.
- Conserto: a recusa vale para o alvo ELEITO pela proximidade, nao como filtro dentro do laco. Pular o consumido e seguir procurando faz cair no candidato mais DISTANTE, que e exatamente o decoy que a regra de proximidade existe para evitar (3a auditoria: alvo errado e pior que nenhum, porque nao casa com claim e o kill sai liberado parecendo conferido). Medido: a versao com `continue` fazia um comando do corpus ganhar `firefox` e `notepad` vindos de texto decorativo.
- Pronto quando: o repro sai `deny`, os tres contrapesos seguem passando (CIM legitimo e `Get-Process X | Stop-Process` continuam identificados pelo nome real; dois alvos explicitos rendem exatamente os dois, sem sentinela), e o diferencial contra corpus real nao perde cobertura.
- Verificacao: 383 testes, 0 falhas. Diferencial sobre 29.582 comandos unicos dos JSONL: 4 divergencias (0,014%), todas ganho de sentinela em heredoc/snippet do proprio trabalho de auditoria, 0 perda de id, 0 excecoes.
